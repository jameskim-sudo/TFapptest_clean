import { useState, useEffect } from "react";
import svgPaths from "@/imports/svg-n2uovsk7bk";
import { ChevronLeft, Camera, X, Calendar } from "lucide-react";
import { getRandomQuestions, Question } from "@/data/questionData";

interface RunningCoachingScreenProps {
  onBackClick: () => void;
  onComplete: () => void;
}

function StatusBar() {
  return (
    <div className="absolute h-[42px] left-0 overflow-clip top-0 w-[360px]">
      <div className="absolute h-[28.8px] left-0 right-0 top-0" />
      <div className="absolute content-stretch flex gap-[3.84px] items-center right-[13.44px] top-[15.36px]">
        {/* Network Signal */}
        <div className="h-[13.44px] relative shrink-0 w-[19.2px]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 19.2 13.44">
            <g>
              <path clipRule="evenodd" d={svgPaths.p1f162900} fill="black" fillRule="evenodd" />
              <path clipRule="evenodd" d={svgPaths.p1d5dbe40} fill="black" fillRule="evenodd" />
              <path clipRule="evenodd" d={svgPaths.p18019e00} fill="black" fillRule="evenodd" />
              <path clipRule="evenodd" d={svgPaths.p342c3400} fill="white" fillOpacity="0.18" fillRule="evenodd" />
            </g>
          </svg>
        </div>
        {/* WiFi Signal */}
        <div className="h-[13.44px] relative shrink-0 w-[15.36px]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15.36 13.44">
            <g>
              <path d={svgPaths.p3dc48e00} fill="black" />
              <path d={svgPaths.p3b3c95f0} fill="black" />
              <path d={svgPaths.p932c700} fill="white" fillOpacity="0.3" />
            </g>
          </svg>
        </div>
        {/* Battery */}
        <div className="h-[13.44px] relative shrink-0 w-[24px]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 25 13.44">
            <g>
              <path d={svgPaths.p693fb00} fill="black" />
              <path d={svgPaths.p38366500} fill="black" fillOpacity="0.6" />
              <path d={svgPaths.p9a22600} fill="black" fillOpacity="0.6" />
              <path d={svgPaths.p5709d00} fill="black" fillOpacity="0.6" />
              <path d={svgPaths.p1fcc0380} fill="#34C759" />
            </g>
          </svg>
        </div>
      </div>
      <div className="absolute h-[20.16px] left-[20.16px] overflow-clip rounded-[19.2px] top-[11.52px] w-[51.84px]">
        <div className="absolute h-[15px] left-[calc(50%+1.58px)] top-[calc(50%+0.42px)] translate-x-[-50%] translate-y-[-50%] w-[33px]">
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 33 15">
            <g>
              <path d={svgPaths.p309cf100} fill="black" />
              <path d={svgPaths.p1285b880} fill="black" />
              <path d={svgPaths.pa9bea00} fill="black" />
              <path d={svgPaths.p1d3f77f0} fill="black" />
            </g>
          </svg>
        </div>
      </div>
    </div>
  );
}

function TopBar({ onBackClick }: { onBackClick: () => void }) {
  return (
    <div className="absolute h-[54px] left-0 overflow-clip top-[42px] w-[360px]">
      <div className="absolute inset-0 bg-white" />
      <button onClick={onBackClick} className="absolute left-[14px] size-[24px] top-[15px]">
        <ChevronLeft className="size-full text-[#272833]" strokeWidth={1.6} />
      </button>
      <p className="absolute font-['Pretendard:Medium',sans-serif] leading-[normal] left-[39.17%] not-italic right-[39.17%] text-[#2e333a] text-[16px] text-center top-[calc(50%-10px)] tracking-[-0.5px] whitespace-pre">
        AI 코칭 LOG
      </p>
    </div>
  );
}

function ProgressBar({ currentStep }: { currentStep: number }) {
  return (
    <div className="absolute flex gap-[5px] left-[20px] top-[111px] w-[320px]">
      {[0, 1, 2, 3, 4, 5].map((step) => (
        <div
          key={step}
          className={`h-[4px] flex-1 ${
            step === currentStep ? "bg-[#f5573f]" : "bg-[#f49b90]"
          }`}
        />
      ))}
    </div>
  );
}

function ClockIcon() {
  return (
    <div className="size-[28px]">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 28 28">
        <g>
          <circle cx="14" cy="14" fill="white" r="14" />
          <g>
            <circle cx="14" cy="14" fill="#4F4F4F" r="11" />
            <circle cx="11" cy="12.3604" fill="black" r="1" />
            <path d="M11 9.03809L13.312 12.3604" stroke="black" />
            <path d={svgPaths.p32ca5e80} fill="white" />
            <circle cx="17.123" cy="15.5801" fill="black" r="1" />
            <path d={svgPaths.p204e1980} stroke="black" />
            <path d={svgPaths.p31453900} fill="#FFAF2E" />
            <rect
              fill="#F5573F"
              height="3.16122"
              rx="0.5"
              transform="rotate(30.6846 10.6133 2)"
              width="19.0886"
              x="10.6133"
              y="2"
            />
          </g>
        </g>
      </svg>
    </div>
  );
}

interface OptionButtonProps {
  label: string;
  isSelected: boolean;
  onClick: () => void;
  showClock?: boolean;
}

function OptionButton({ label, isSelected, onClick, showClock }: OptionButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`relative h-[52px] w-full rounded-[8px] border border-[#ff482c] flex items-center px-6 ${
        isSelected ? "bg-[#f5573f]" : "bg-white"
      }`}
    >
      <p
        className={`font-['Pretendard:Bold',sans-serif] text-[16px] tracking-[-0.5px] whitespace-nowrap overflow-hidden text-ellipsis ${
          isSelected ? "text-white" : "text-[#292929]"
        }`}
      >
        {label}
      </p>
      {isSelected && (
        <div className="absolute right-[12px]">
          <ClockIcon />
        </div>
      )}
    </button>
  );
}

interface MultiOptionButtonProps {
  label: string;
  isSelected: boolean;
  onClick: () => void;
  showClock?: boolean;
}

function MultiOptionButton({ label, isSelected, onClick, showClock }: MultiOptionButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`relative h-[52px] w-[155px] rounded-[8px] border border-[#ff482c] flex items-center justify-center px-4 ${
        isSelected ? "bg-[#f5573f]" : "bg-white"
      }`}
    >
      <p
        className={`font-['Pretendard:Bold',sans-serif] text-[16px] tracking-[-0.5px] whitespace-nowrap overflow-hidden text-ellipsis ${
          isSelected ? "text-white" : "text-[#292929]"
        }`}
      >
        {label}
      </p>
      {isSelected && (
        <div className="absolute right-[8px]">
          <ClockIcon />
        </div>
      )}
    </button>
  );
}

interface MediaFile {
  id: string;
  file: File;
  preview: string;
  type: 'image' | 'video';
}

export default function RunningCoachingScreen({ onBackClick, onComplete }: RunningCoachingScreenProps) {
  const [currentStep, setCurrentStep] = useState(0);
  const [questions, setQuestions] = useState<Question[]>([]);
  const [answers, setAnswers] = useState<(string | string[] | number)[]>([null, null, null, null]);
  const [detailInput, setDetailInput] = useState("");
  const [mediaFiles, setMediaFiles] = useState<MediaFile[]>([]);
  const [selectedDate, setSelectedDate] = useState<string | null>(null);

  // 컴포넌트 마운트 시 랜덤 질문 가져오기
  useEffect(() => {
    const randomQuestions = getRandomQuestions('running');
    setQuestions(randomQuestions);
  }, []);

  const handleNext = () => {
    if (currentStep < 5) {
      setCurrentStep(currentStep + 1);
      setDetailInput(""); // 다음 질문으로 이동 시 입력창 초기화
    } else {
      // 마지막 단계 - 답변 저장 및 완료
      console.log('Running coaching answers:', { selectedDate, questions, answers, mediaFiles });
      onComplete();
    }
  };

  const handleSingleSelect = (value: string) => {
    const newAnswers = [...answers];
    newAnswers[currentStep - 1] = value;
    setAnswers(newAnswers);
  };

  const handleMultiSelect = (value: string) => {
    const newAnswers = [...answers];
    const currentAnswer = (newAnswers[currentStep - 1] as string[]) || [];
    
    if (currentAnswer.includes(value)) {
      newAnswers[currentStep - 1] = currentAnswer.filter((item) => item !== value);
    } else {
      newAnswers[currentStep - 1] = [...currentAnswer, value];
    }
    setAnswers(newAnswers);
  };

  const handleScaleSelect = (value: number) => {
    const newAnswers = [...answers];
    newAnswers[currentStep - 1] = value;
    setAnswers(newAnswers);
  };

  const handleMediaUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files) return;

    Array.from(files).forEach((file) => {
      const fileType = file.type.startsWith('image/') ? 'image' : 'video';
      const preview = URL.createObjectURL(file);
      
      setMediaFiles((prev) => [
        ...prev,
        {
          id: Math.random().toString(36).substring(7),
          file,
          preview,
          type: fileType,
        },
      ]);
    });
    
    // Reset input
    e.target.value = '';
  };

  const handleRemoveMedia = (id: string) => {
    setMediaFiles((prev) => {
      const fileToRemove = prev.find((f) => f.id === id);
      if (fileToRemove) {
        URL.revokeObjectURL(fileToRemove.preview);
      }
      return prev.filter((f) => f.id !== id);
    });
  };

  // Cleanup on unmount
  useEffect(() => {
    return () => {
      mediaFiles.forEach((file) => {
        URL.revokeObjectURL(file.preview);
      });
    };
  }, []);

  if (questions.length === 0) {
    return (
      <div className="bg-white relative size-full">
        <StatusBar />
        <TopBar onBackClick={onBackClick} />
      </div>
    );
  }

  const currentQuestion = questions[currentStep - 1]; // Step 0은 날짜 선택이므로 -1
  const currentAnswer = answers[currentStep - 1];

  const canProceed = () => {
    // 날짜 선택 단계 (step 0)
    if (currentStep === 0) {
      return selectedDate !== null;
    }
    
    // 미디어 추가 단계 (step 5)
    if (currentStep === 5) {
      return true; // 미디어는 선택사항
    }
    
    if (currentQuestion.type === 'multi') {
      return Array.isArray(currentAnswer) && currentAnswer.length > 0;
    }
    return currentAnswer !== null && currentAnswer !== undefined;
  };

  // Generate dates for calendar
  const generateDates = () => {
    const dates = [];
    const today = new Date(2026, 0, 29); // Jan 29, 2026 (Monday)
    const year = 2026;
    const month = 0; // January (0-indexed)
    
    // Get first day of month and total days in month
    const firstDay = new Date(year, month, 1);
    const lastDay = new Date(year, month + 1, 0);
    const daysInMonth = lastDay.getDate();
    const startDayOfWeek = firstDay.getDay(); // 0 = Sunday

    // Add empty cells for days before month starts
    for (let i = 0; i < startDayOfWeek; i++) {
      dates.push(null);
    }

    // Add all days of the month
    for (let date = 1; date <= daysInMonth; date++) {
      const currentDate = new Date(year, month, date);
      const dayOfWeek = ['일', '월', '화', '수', '목', '금', '토'][currentDate.getDay()];
      const isToday = currentDate.toDateString() === today.toDateString();
      const isFutureDate = currentDate > today;
      const dateString = `2026.${String(month + 1).padStart(2, '0')}.${String(date).padStart(2, '0')}`;
      
      // Determine color based on day of week
      let dayColor = '#292929';
      if (currentDate.getDay() === 0) dayColor = '#ff482c'; // Sunday
      if (currentDate.getDay() === 6) dayColor = '#004fe8'; // Saturday
      
      dates.push({
        date,
        dayOfWeek,
        isToday,
        isFutureDate,
        dateString,
        dayColor,
        displayDate: date.toString()
      });
    }

    return dates;
  };

  const dates = generateDates();

  return (
    <div className="bg-white relative size-full">
      <StatusBar />
      <TopBar onBackClick={onBackClick} />
      <ProgressBar currentStep={currentStep} />

      {/* Date Selection Step (Step 0) */}
      {currentStep === 0 ? (
        <>
          {/* Title */}
          <div className="absolute flex flex-col font-['Pretendard:Bold',sans-serif] justify-center leading-[0] left-[calc(50%-160px)] not-italic text-[#ff482c] text-[20px] top-[156.5px] tracking-[1px] translate-y-[-50%] whitespace-nowrap">
            <p className="leading-[23px] whitespace-pre">운동 날짜 선택</p>
          </div>
          <p className="absolute font-['Pretendard:Medium',sans-serif] leading-[20px] left-[calc(50%-160px)] not-italic text-[#292929] text-[14px] top-[178px] tracking-[-0.5px] whitespace-pre">
            기록할 운동의 날짜를 선택해주세요
          </p>

          {/* Date Calendar */}
          <div className="absolute left-[20px] top-[218px] w-[320px]">
            <div className="bg-white border border-[#ff482c] rounded-[12px] p-[12px]">
              <div className="flex items-center justify-center gap-[8px] mb-[12px]">
                <p className="font-['Pretendard:Bold',sans-serif] text-[15px] text-[#292929]">
                  2026년 1월
                </p>
              </div>
              
              {/* Day of Week Header */}
              <div className="grid grid-cols-7 gap-[6px] mb-[6px]">
                {['일', '월', '화', '수', '목', '금', '토'].map((day, index) => (
                  <div key={day} className="flex items-center justify-center">
                    <span 
                      className="font-['Pretendard:Bold',sans-serif] text-[11px]"
                      style={{ 
                        color: index === 0 ? '#ff482c' : index === 6 ? '#004fe8' : '#292929' 
                      }}
                    >
                      {day}
                    </span>
                  </div>
                ))}
              </div>
              
              {/* Date Grid */}
              <div className="grid grid-cols-7 gap-[4px]">
                {dates.map((dateInfo, index) => {
                  if (!dateInfo) {
                    return <div key={index} />;
                  }
                  
                  return (
                    <button
                      key={index}
                      onClick={() => !dateInfo.isFutureDate && setSelectedDate(dateInfo.dateString)}
                      disabled={dateInfo.isFutureDate}
                      className={`flex flex-col items-center justify-center py-[4px] rounded-[8px] transition-all ${
                        dateInfo.isFutureDate ? 'cursor-not-allowed opacity-30' : 'cursor-pointer'
                      }`}
                    >
                      {/* Date */}
                      <div className={`flex items-center justify-center w-[26px] h-[26px] rounded-full transition-all ${
                        dateInfo.isToday && !selectedDate
                          ? 'bg-[#292929]'
                          : selectedDate === dateInfo.dateString
                          ? 'bg-[#ff482c]'
                          : ''
                      }`}>
                        <span 
                          className={`font-['Pretendard:Bold',sans-serif] text-[13px] ${
                            dateInfo.isToday && !selectedDate
                              ? 'text-white'
                              : selectedDate === dateInfo.dateString
                              ? 'text-white'
                              : ''
                          }`}
                          style={{ 
                            color: (!dateInfo.isToday && selectedDate !== dateInfo.dateString) ? dateInfo.dayColor : undefined 
                          }}
                        >
                          {dateInfo.displayDate}
                        </span>
                      </div>
                    </button>
                  );
                })}
              </div>
            </div>

            {/* Selected Date Display */}
            {selectedDate && (
              <div className="mt-[12px] bg-[#fff5f4] border border-[#ff482c] rounded-[8px] p-[10px] flex items-center justify-between">
                  <p className="font-['Pretendard:Medium',sans-serif] text-[13px] text-[#292929]">
                    선택된 날짜
                  </p>
                  <p className="font-['Pretendard:Bold',sans-serif] text-[13px] text-[#ff482c]">
                    {selectedDate}
                  </p>
              </div>
            )}
          </div>
        </>
      ) : currentStep === 5 ? (
        <>
          {/* Media Upload Step (Step 5) */}
          <div className="absolute flex flex-col font-['Pretendard:Bold',sans-serif] justify-center leading-[0] left-[calc(50%-160px)] not-italic text-[#ff482c] text-[20px] top-[156.5px] tracking-[1px] translate-y-[-50%] whitespace-nowrap">
            <p className="leading-[23px] whitespace-pre">운동 사진/영상</p>
          </div>
          <p className="absolute font-['Pretendard:Medium',sans-serif] leading-[20px] left-[calc(50%-160px)] not-italic text-[#292929] text-[14px] top-[178px] tracking-[-0.5px] whitespace-pre">
            오늘의 운동을 기록해보세요 (선택사항)
          </p>

          {/* Upload Button */}
          <label className="absolute left-[20px] top-[228px] w-[320px] h-[100px] border-2 border-dashed border-[#ff482c] rounded-[8px] flex flex-col items-center justify-center cursor-pointer hover:bg-[#fff5f4] transition-colors">
            <input
              type="file"
              accept="image/*,video/*"
              multiple
              onChange={handleMediaUpload}
              className="hidden"
            />
            <Camera className="size-[32px] text-[#ff482c] mb-2" />
            <p className="font-['Pretendard:Medium',sans-serif] text-[14px] text-[#ff482c] tracking-[-0.5px]">
              사진/영상 추가하기
            </p>
          </label>

          {/* Media Preview */}
          {mediaFiles.length > 0 && (
            <div className="absolute left-[20px] top-[348px] w-[320px] max-h-[170px] overflow-y-auto">
              <div className="grid grid-cols-3 gap-2">
                {mediaFiles.map((media) => (
                  <div key={media.id} className="relative aspect-square rounded-[8px] overflow-hidden bg-gray-100">
                    {media.type === 'image' ? (
                      <img
                        src={media.preview}
                        alt="preview"
                        className="w-full h-full object-cover"
                      />
                    ) : (
                      <video
                        src={media.preview}
                        className="w-full h-full object-cover"
                      />
                    )}
                    <button
                      onClick={() => handleRemoveMedia(media.id)}
                      className="absolute top-1 right-1 size-[20px] bg-black/60 rounded-full flex items-center justify-center hover:bg-black/80"
                    >
                      <X className="size-[14px] text-white" />
                    </button>
                  </div>
                ))}
              </div>
            </div>
          )}
        </>
      ) : (
        <>
          {/* Question Title */}
          <div className="absolute flex flex-col font-['Pretendard:Bold',sans-serif] justify-center leading-[0] left-[calc(50%-160px)] not-italic text-[#ff482c] text-[20px] top-[156.5px] tracking-[1px] translate-y-[-50%] whitespace-nowrap">
            <p className="leading-[23px] whitespace-pre">{currentQuestion.title}</p>
          </div>
          <p className="absolute font-['Pretendard:Medium',sans-serif] leading-[20px] left-[calc(50%-160px)] not-italic text-[#292929] text-[14px] top-[178px] tracking-[-0.5px] whitespace-pre">
            {currentQuestion.description}
          </p>

          {/* Single Select */}
          {currentQuestion.type === 'single' && (
            <div className="absolute left-[20px] top-[228px] w-[320px] space-y-[10px]">
              {currentQuestion.options?.map((option, index) => (
                <OptionButton
                  key={option.value}
                  label={option.label}
                  isSelected={currentAnswer === option.value}
                  onClick={() => handleSingleSelect(option.value)}
                  showClock={index === 1}
                />
              ))}
            </div>
          )}

          {/* Scale (5 points) */}
          {currentQuestion.type === 'scale' && (
            <div className="absolute left-[20px] top-[228px] w-[320px] space-y-[10px]">
              {currentQuestion.scaleRange?.labels?.map((label, index) => (
                <OptionButton
                  key={index + 1}
                  label={label}
                  isSelected={currentAnswer === index + 1}
                  onClick={() => handleScaleSelect(index + 1)}
                  showClock={index === 2}
                />
              ))}
            </div>
          )}

          {/* Multi Select */}
          {currentQuestion.type === 'multi' && (
            <>
              <div className="absolute left-[20px] top-[228px] w-[320px]">
                {currentQuestion.options && currentQuestion.options.length === 4 && (
                  <>
                    <div className="flex gap-[10px] mb-[10px]">
                      <MultiOptionButton
                        label={currentQuestion.options[0].label}
                        isSelected={Array.isArray(currentAnswer) && currentAnswer.includes(currentQuestion.options[0].value)}
                        onClick={() => handleMultiSelect(currentQuestion.options[0].value)}
                        showClock
                      />
                      <MultiOptionButton
                        label={currentQuestion.options[1].label}
                        isSelected={Array.isArray(currentAnswer) && currentAnswer.includes(currentQuestion.options[1].value)}
                        onClick={() => handleMultiSelect(currentQuestion.options[1].value)}
                      />
                    </div>
                    <div className="flex gap-[10px]">
                      <MultiOptionButton
                        label={currentQuestion.options[2].label}
                        isSelected={Array.isArray(currentAnswer) && currentAnswer.includes(currentQuestion.options[2].value)}
                        onClick={() => handleMultiSelect(currentQuestion.options[2].value)}
                      />
                      <MultiOptionButton
                        label={currentQuestion.options[3].label}
                        isSelected={Array.isArray(currentAnswer) && currentAnswer.includes(currentQuestion.options[3].value)}
                        onClick={() => handleMultiSelect(currentQuestion.options[3].value)}
                        showClock
                      />
                    </div>
                  </>
                )}
              </div>

              {/* Text Input - only show for 3rd and 4th questions when "기타" is selected */}
              {(currentStep === 3 || currentStep === 4) && Array.isArray(currentAnswer) && currentAnswer.includes('기타') && (
                <>
                  <p className="absolute font-['Pretendard:Medium',sans-serif] leading-[20px] left-[calc(50%-160px)] not-italic text-[#292929] text-[14px] top-[372px] tracking-[-0.5px] whitespace-pre">
                    상세 내용 입력 (최대 100자)
                  </p>
                  <div className="absolute left-[calc(50%-160px)] top-[402px] w-[320px]">
                    <div className="relative bg-white border border-[#ff482c] rounded-[8px] h-[111px] p-4">
                      <textarea
                        value={detailInput}
                        onChange={(e) => {
                          if (e.target.value.length <= 100) {
                            setDetailInput(e.target.value);
                          }
                        }}
                        placeholder="상세 내용을 입력해주세요"
                        className="w-full h-full resize-none bg-transparent font-['Pretendard:Medium',sans-serif] text-[12px] text-[#292929] tracking-[-0.5px] outline-none placeholder:text-[#999]"
                      />
                    </div>
                    <p className="font-['Pretendard:Medium',sans-serif] text-[12px] text-[#292929] tracking-[-0.5px] text-right mt-2">
                      {detailInput.length} / 100
                    </p>
                  </div>
                </>
              )}
            </>
          )}
        </>
      )}

      {/* Next Button */}
      <button
        onClick={handleNext}
        disabled={!canProceed()}
        className={`absolute bg-[#f5573f] h-[52px] left-[20px] rounded-[30px] top-[548px] w-[320px] flex items-center justify-center transition-opacity ${
          !canProceed() ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#e04a35] cursor-pointer'
        }`}
      >
        <p className="font-['Pretendard:Medium',sans-serif] text-[16px] text-white tracking-[-0.5px]">
          {currentStep < 4 ? '다음' : '완료'}
        </p>
      </button>
    </div>
  );
}