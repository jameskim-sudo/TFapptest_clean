...
interface PilatesCoachingScreenProps {
  onBackClick: () => void;
  onComplete: () => void;
}
...
export default function PilatesCoachingScreen({ onBackClick, onComplete }: PilatesCoachingScreenProps) {
  ...
  useEffect(() => {
    const randomQuestions = getRandomQuestions('pilates');
  ...
  const handleNext = () => {
    if (currentStep < 5) {
      ...
      // 마지막 단계 - 답변 저장 및 완료
      console.log('Pilates coaching answers:', { selectedDate, questions, answers, mediaFiles });
      onComplete();
    }
  };
