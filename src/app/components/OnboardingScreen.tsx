...
function Group2() {
  return (
    <div className="absolute contents left-[20px] top-[62px]">
      <Group1 />
      <div className="absolute flex flex-col font-[Pilseung_Gothic] justify-center leading-[0] left-[calc(50%-127px)] not-italic text-[#ff482c] text-[20px] top-[72px] tracking-[1px] translate-y-[-50%] whitespace-nowrap">
...
export default function OnboardingScreen({ onStartClick, onLoginClick }: OnboardingScreenProps) {
  return (
    <div className="bg-white relative size-full" data-name="360x640">
      ...
      <StatusBar />
      <Group2 />
      <div className="absolute flex flex-col font-[Pilseung_Gothic] justify-center leading-[0] left-1/2 not-italic text-[#ff482c] text-[25px] text-center top-[404px] tracking-[1px] translate-x-[-50%] translate-y-[-50%] whitespace-nowrap">
        <p className="leading-[20px] whitespace-pre">Smart AI Coach</p>
      </div>
      <button
        onClick={onStartClick}
        className="absolute bg-[#f5573f] content-stretch flex h-[52px] items-center justify-center left-[20px] px-[127px] py-[16px] rounded-[30px] top-[520px] w-[320px] cursor-pointer hover:bg-[#e04a35] transition-colors"
      >
        <div className="content-stretch flex items-start">
          <p className="font-['Pretendard:Medium',sans-serif] leading-[normal] not-italic shrink-0 text-[16px] text-white tracking-[-0.5px] whitespace-pre">운동 기록 시작하기</p>
          <Icon />
        </div>
      </button>
      <div className="absolute font-['Pretendard:Medium',sans-serif] leading-[20px] left-1/2 not-italic text-[#292929] text-[0px] text-[14px] text-center top-[437px] tracking-[-0.5px] translate-x-[-50%] whitespace-nowrap whitespace-pre">
        <p className="mb-0">골프 · 필라테스 · 러닝</p>
        <p>
          <span className="font-['Pretendard:SemiBold',sans-serif] not-italic">AI 기반</span>
          <span>{` 정밀 모션 분석`}</span>
        </p>
      </div>
      <p className="absolute font-['Pretendard:Medium',sans-serif] leading-[18px] left-[calc(50%+0.5px)] not-italic text-[#292929] text-[12px] text-center top-[592px] tracking-[-0.5px] translate-x-[-50%] whitespace-pre">
        <span>{`이미 계정이 있으신가요?  `}</span>
        <button
          onClick={onLoginClick}
          className="[text-decoration-skip-ink:none] [text-underline-position:from-font] decoration-solid font-['Pretendard:Bold',sans-serif] text-[#ff482c] underline cursor-pointer hover:opacity-80 transition-opacity"
        >
          로그인
        </button>
      </p>
